...
##			Version 1.2.3 (last modified on 04-JAN-2019)				##
...
multivariate_GWAMA<-function(	x,cov_Z=NULL,h2,
	...
	cat("done\n")
	#nrows<-lapply(x,nrow)
	#cat("  Input contained to following number of SNPs:\n")
	#print(nrows)
	...
	#	nrows<-lapply(x,function(x){return(0)})
	#}else{
	#	for(i in 1:length(x)){
	#		if(length()==0){
	#			nrows[[i]]<-0
	...
	#			x[[i]]<-dum
	#			nrows[[i]]<-nrows[[i]]-nrow(dum)
	#			rm(dum)
	#		}
	#	}
	#}
	cat("done\n")
	#cat("  The following number of SNPs were removed:\n")
	#print(nrows)
	...
	sumstats<-cbind(NA,SNP_info,EAF_multi,MAF,N_multi,direction,B_multi,SE_multi,Z_multi,P_multi)
